...
#include <BLEUtils.h>
#include <BLEServer.h>
#include <BLE2901.h> // Native User Descriptor Support
#include <esp_sleep.h>
...
#define CHAR_PAUSE_UUID     "4fafc209-1fb5-459e-8fcc-c5c9c331914b" // New Pause/Resume UUID

// 100% Verified Hardware GPIO Architecture Matrix
...
bool timerSuspended = false;       // Master execution timer toggle state
unsigned long sessionStartTime = 0;
unsigned long lastSampleTime = 0;
unsigned long lastBatterySampleTime = 0;        
unsigned long lastSuspendUpdate = 0; // Tracks intervals during suspension
    ...
    void onWrite(BLECharacteristic *pCharacteristic) {
      ...
      if (value.length() > 0) {
        if ((value == "1" || value.charAt(0) == 0x01)) { 
          if (uuid == CHAR_RESET_UUID) {
            resetRequested = true;
          } 
          else if (uuid == CHAR_TERM_UUID) {
            terminateSession = true;
          }
          else if (uuid == CHAR_EXTEND_UUID) {
            extendRequested = true;
          }
          else if (uuid == CHAR_PAUSE_UUID) {
            timerSuspended = true; // Suspend the countdown
          }
        }
        else if ((value == "0" || value.charAt(0) == 0x00)) {
          if (uuid == CHAR_PAUSE_UUID) {
            timerSuspended = false; // Resume the countdown
          }
        }
      }
    }
...
void setup() {
  ...
  BLECharacteristic *pPauseChar  = pService->createCharacteristic(CHAR_PAUSE_UUID,  BLECharacteristic::PROPERTY_WRITE); // New Pause/Resume Characteristic
  
  ControlCallbacks* callbacks = new ControlCallbacks();
  pResetChar->setCallbacks(callbacks);
  pTermChar->setCallbacks(callbacks);
  pExtendChar->setCallbacks(callbacks); 
  pPauseChar->setCallbacks(callbacks); // Attach callback for pause/resume

  addLabel(pCurrChar,   "Current Temperature (C)");
  addLabel(pMaxChar,    "Maximum Temperature (C)");
  addLabel(pMinChar,    "Minimum Temperature (C)");
  addLabel(pBatChar,    "Battery Voltage (V)");
  addLabel(pResetChar,  "Write 1 to Reset Metrics");
  addLabel(pTermChar,   "Write 1 to Sleep Device");
  addLabel(pExtendChar, "Write 1 to Add 2 Mins to Session");
  addLabel(pPauseChar,  "Write 1 to Pause Timer, 0 to Resume");
...
void loop() {
  ...
  // Physical Reed Switch Override (Always works for safety)
  ...
  // Handle Dynamic Time Extensions
  if (extendRequested) {
  ...
  // Timer Control Logic
  if (timerSuspended) {
    // Calculate elapsed time since the last calculation loop and push the start time forward
    unsigned long timeElapsed = currentMillis - lastSuspendUpdate;
    sessionStartTime += timeElapsed; 
  }
  lastSuspendUpdate = currentMillis; // Keep tracking absolute delta baseline

  // Evaluate execution window threshold against adjusted baseline
  if (currentMillis - sessionStartTime >= sessionDuration) {
    enterUltraLowPowerSleep();
  }
  ...
  // 1. Temperature Loop (Runs every 10 seconds independent of BLE/Timer status)
  ...
  // 2. Battery Loop (Runs every 30 seconds independent of BLE/Timer status)
